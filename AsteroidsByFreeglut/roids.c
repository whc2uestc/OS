...
void roid_draw( const roid_t* roid )
{
  ...
  if(roid->x-roid->width/2.0 < 0 && roid->y-roid->height/2.0 > 1){
  	draw_rectangle( 1+roid->x-roid->width/2.0, roid->y-roid->height/2.0-1, 
		  1, 0,
		  roid->color );
  }
  if(roid->x-roid->width/2.0 > 1 && roid->y-roid->height/2.0 < 0){
  	draw_rectangle( roid->x-roid->width/2.0-1, 1+roid->y-roid->height/2.0, 
		  0, 1,
		  roid->color );
  }
  ...
  if(roid->x-roid->width/2.0 < 0){
  	draw_rectangle( 1+roid->x-roid->width/2.0, roid->y-roid->height/2.0, 
		  1, roid->y+roid->height/2,
		  roid->color );
  } 
  if(roid->y-roid->height/2.0 < 0){
  	draw_rectangle( roid->x-roid->width/2.0, 1+roid->y-roid->height/2.0, 
		  roid->x+roid->width/2.0, 1,
		  roid->color );
  }
  if(roid->x+roid->width/2.0 > 1){
  	draw_rectangle( 0, roid->y-roid->height/2.0, 
		  roid->x+roid->width/2.0-1, roid->y+roid->height/2,
		  roid->color );
  }
  if(roid->y+roid->height/2.0 > 1){
  	draw_rectangle( roid->x-roid->width/2.0, 0, 
  ...
  if(roid->y - roid->height/2 < 0){
  	if (( shot->x >= roid->x - roid->width/2 &&
	 	shot->x <= roid->x + roid->width/2 &&
  		...
  		shot->x >= roid->x - roid->width/2 &&
	 	shot->x <= roid->x + roid->width/2 &&
	  	shot->y-1 >= roid->y - roid->height/2 &&
	  	shot->y-1 <= roid->y + roid->height/2 ))
		return 1;
  }
  if(roid->x + roid->width/2 > 1){
  	if(( shot->x >= roid->x - roid->width/2 &&
	 	shot->x <= roid->x + roid->width/2 &&
	  	shot->y >= roid->y - roid->height/2 &&
	  	shot->y <= roid->y + roid->height/2 ) || (
...
void key_down( unsigned char key, int x, int y )
{
  /* TASK 1 */
  switch(key){
  	case 'z': ship.da = SHIP_SPIN_SPEED; break;
	case 'x': ship.da = -SHIP_SPIN_SPEED; break;
	case 'p': paused = paused==0?1:0; break;
	case 'm': ship.thrust = 1; break;
	case ' ': shot_add(); break;
  }
  /* TODO:
...
void key_up( unsigned char key, int x, int y )
{
  /* TASK 2 */
  switch(key){
	case 'z': ship.da = 0; break;
	case 'x': ship.da = 0; break;
	case 'm': ship.thrust = 0; break;
  }
  /* TODO:     
     using a switch/case statement
